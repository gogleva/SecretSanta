...
m_slicer <- function(input_object, length_threshold, run_mode) {
                    ...
                    # check that inputs are valid - need to split validation and assignments 
                    
                    if (is(input_object, 'AAStringSet') && (run_mode != 'slice')) {
                    ...
                    if (is(input_object, 'CBSResult')) {
                      infa <- getInfasta(input_object)
                      outfa <- getOutfasta(input_object)
                      input_object <- infa[names(infa) %!in% names(outfa)]
                      }
