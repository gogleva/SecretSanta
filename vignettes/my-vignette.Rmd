...
For prediction of secreted proteins SecretSanta relies on a set of existing [CBS tools](http://www.cbs.dtu.dk/services/). Please install them and configure according to the listed requirements. ``SecretSanta`` provides R intreface for the following tools:
...
- **signalp-2.0** 
    ...
    including the legacy ones, it is important to be able to duscriminate between
    them. R is oblivious to shell aliaces, so we will simply rename the **siganlp**
    script:
...
- **WoLFPsort**
    ...
    + The most important script we need **runWolfPsortSummary** has a bulky name,
    we will rename it to simply **wolfpsort** for the future convinience:
...
In order to pipe result from differenr versions of **signalp** just switch to the ``run_mode = 'piper'`` for the second and other downstream steps. In this case
``signalp`` will run the anlysis using contents of ``out_fasta`` slot as an input.
Say, we want to do the following piping: **signalp2** -> **signalp3** -> **signalp4**.
...
``tmhmm`` function can handle input objects of SignalpResult class with non-empty ```mature_fasta``` slot (see starters and pipers table for allowed inputs).
...
``targetp`` requires specifying ``network_type`` to run correctly. Possible values include:
...
sequences and remove those with ER retention signal in the C-terminus. To run it,
simply pass a ``CBSResult`` object as an input.  You can run it in either *piper*
or *starter* mode. The function does not rely on external dependencies, so
providing path container is not required.
...
slotNames(er_result)
```
+ ``in_fasta`` and ``out_fasta`` are common attriburtes of all classes inheriting
from ``CBSResult`` class;
...
generated slices are  build by concatenating original sequence id, 'slice' string
and position of the Methionine sliced from. For example: ``ALI_PLTG_3_slice_M86`` 
...
+ ``rescue`` - having output from any other up-stream function it extracts proteins not predicted to be secreted on the initial run and generate slices for them.
...
<!-- Now, we want to build a pipeline using all our available functions. Say, we would like to have a relatively short one with the following steps: -->
...
<!-- This is an example of a fairly simple pipeline, but you can create more complicated ones. The stringent pipeline might look like this and involve stringent filatration on signal peptide prediction as well as additional predictions for 'rescued' proteins: -->
