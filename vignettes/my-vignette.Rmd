...
**SecretSanta** provides R interface for integrative prediction of secreted
proteins based on amino acid sequences. Secretome prediction often involves multiple steps. It usually starts with prediction of short signal peptides at
the N-terminus end of a protein and potential cleavage sites. The tool of choice
here is signalp (ref). Next, it is important to check the presence of transmembrane domains in the mature protein sequence (obtained after clevage of signal peptide),
to make sure that the protein won't be retined in the membrane. 

Though a number of excellent tools exist
() an interface allowing to easly pipe outputs

The 

R interface for a family of [CBS tools](http://www.cbs.dtu.dk/services/). The wrappers are designed to work together. They produce output in a format recognosible by all of them. This 
alows to pipe results between individual predictors easily, as well as create flexible custom pipelines and compare outputs between several tools. 

To speed-up processing of large input fasta files (with several thousands of sequences) reccommended initial steps of the pipeline are automatically run as embarassingly parallel processes when the number of input sequences exceedes a certain limit.

To avoid confusion, here and after names of external tools will be in bold, corresponding R functions from the SecretSanta package would have standard code highlighting. For example:
