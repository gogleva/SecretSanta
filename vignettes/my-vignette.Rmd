...
fig_caption: yes
---  
**SecretSanta** provides an R interface for integrative prediction of classically secreted proteins. 
...
mitochondria and plastid targeting signals (**Figure 1, c-e**). 

```{r figs2, out.width = "400px", fig.align="center", echo = FALSE, fig.cap="\\label{fig:figs1} Figure 1. Motifs and domains"}
knitr::include_graphics("motifs_and_domains.png")
```

Though, a number of excellent command line tools and web-interfaces exist to perform prediction of individual motifs and signal sequences (signalp, targetp, TMHMM, WolfPsort), the interface allowing to combine the outputs in a single flexible workflow is lacking.
...
together. They produce the output in a standardized format recognizable by all
of them. This allows to pipe results between individual predictors easily to
create flexible custom pipelines as well as compare predictions outputs between similar methods. To speed-up processing of large input fasta files (with several thousands of sequences) recommended initial steps of the pipeline are automatically run as embarrassingly parallel processes when the number of input sequences exceeds a certain limit. Taken together **SecretSanta** provides
...
+ *piper* - for downstream/intermediate steps, so that the tool expects an output from another wrapper function.

## 1. Installation of external dependencies

For prediction of secreted proteins SecretSanta relies on a set of existing
[CBS tools](http://www.cbs.dtu.dk/services/). Please install them and configure according to the listed requirements. ``SecretSanta`` provides R interface for
the following tools:
...
- **signalp-3.0** 
    ...
    and ghostview are not required. For more details please check ```signalp-3.0.readme```.
