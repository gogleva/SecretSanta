...
To check that all the required dependencies are installed correctly and can be executed in principle, run ```manage_paths``` function. Note, that ```manage_paths``` function is case insensitive and converts all the tool names to the lower case. 

For CBS tool names please use simple character strings without version numbers. The only exception is signalp family of tools, they require version number in one-digit format in the first column to be distinguishable.

Please note tnat aliases will not work when tools are called from R and changing ```$PATH``` variable is not necessary.
