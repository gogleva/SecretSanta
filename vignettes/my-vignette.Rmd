...
would like to have a relatively short pipeline with the following steps:
...
Now we will run the pipeline using ``SecretSanta`` functions, starting with
...
filtration on signal peptide prediction as well as additional predictions for 'rescued' (**Figure 2**) sequences.

```{r pipilines, out.width = "400px", fig.align="center", echo = FALSE, fig.cap="\\label{fig:pipelines} **Figure 2.** Example of a stringent pipeline."}
