...
To speed-up processing of large input fasta files (with several thousands of sequences) reccommended initial steps of the pipeline (```singalp``` and ```targetp```) are automatically run as embarassingly parallel processes when number of input sequences exceedes a certain limit (600 - for ```signalp```; 500 - for ```targetp```).
...
Signalp is a softaware tool to predict classical signal peptides and cleavage sites [@Nielsen1997] in eukaryotes nad bacteria. The method combines prediction of signal peptides and cleavage cites based on a combination of artificial neural networks as well as hidden Markov models. The latter allows to distinguish between signal peptides and non-cleaved signal anchors. 

Currently SecretSanta provides an interface for the three recent versions of signalp. The reason for this - signalp4 - seems to be a bit to stringent for secreted proteins from oomycetes and some other organisms(?)
