...
input sequences exceeds a certain limit. The package also contains a convenient `ask_uniprot()` function - to export known information about subcellular localisation. This allows the user to compare and verify predictions of extracellular proteins, however is mostly applicable for well-annotated and curated genomes.
...
### 4.2 Tmhmm()
**TMHMM** predicts transmembrane α-helices and identifies integral
...
### 4.4 Targetp()
**TargetP** predicts the subcellular localization of secreted eukaryotic proteins based on the presence of signal peptide (SP), chloroplast transit peptides (cTP) or mitochondrial targeting peptides (mTP) in the N-terminus [@Emanuelsson2000]. 
...
```{r use case model sp, eval = FALSE}
