...
# TOPCONS predictions for P.infestans, we can't include the whole archive in the package due to it's size. 
top_dir <- "rst_7gaVh2.zip"
...
To demonstarte how **SecretSanta** pipelines could be applied for large-scale analysis of proteomes, we will download all available fungal proteomes using **biomartr** package [@Drost2017] and apply the same standard secretome prediction pipeline. To speed-up the process, we will use 20 cores available on our local HPC. Please note, retrieval of proteomes might take a while.
