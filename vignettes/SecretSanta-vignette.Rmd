...
together by producing standardized output as an instance of ``CBSResult`` class.
...
output from another wrapper function. For detailed information on piper/starter behavior please see Table 2.
...
```{r topcons example, collapse=TRUE}
...
+ ``tp_tibble`` - parsed **TargetP** tabular output for candidates not targeted
...
### 4.5 Wolfpsort()
**WoLF PSORT** - predicts protein subcellular localization based on the PSORT principle. It converts amino acid sequences into numerical localization features based on sorting signals, amino acid composition and functional motifs.
The converted data is then classified based on k-nearest neighbor algorithm [@Horton2006].
...
wlf
```
+ ``in_fasta`` and ``out_fasta``; 
+ ``wolf_tibble`` - parsed **WoLF PSORT** tabular output for sequences with 
...
The ``check_khdel()`` uses pattern matching, according to the specified definition to scan amino acid sequences and remove those with an ER retention signal in the C-terminus. To run
it, simply pass a ``CBSResult`` object as an input.  
``check_khdel()`` is restricted to piper-like behavior, because it makes the most sense to check for ER-retention signals in proteins with already predicted signal peptides.
...
+ ``in_fasta`` and ``out_fasta``;
+ ``retained_fasta`` - all the sequences with C-terminal '(K/H)DEL' motifs, in case you would like to have a look at them.
...
``ALI_PLTG_3_slice_M86`` means that ``ALI_PLTG_3`` sequence was sliced starting from the methionine in the position 86.
...
``ask_uniprot()`` is a convenience function, allowing to retrieve information on subcellular localisation of proteins from [UniProtKB](http://www.uniprot.org/help/uniprotkb) in order to compare results obtained within a **SecretSanta** pipeline with available information on subcellular localisation. This function is mostly relevant for well-annotated and curated genomes. 
``ask_uniprot()`` currently requires a simple list of UniprotKB ids and returns information about subcellular localisation, key words as well as relevant GO terms organised in a tibble:
...
In this exmple we will download proteome for *Phytophthora infestans* from the UniprotKB, construct a pipeline for secretome prediction and compare results with known annotations. We will use **TOPCONS** for prediction of transmembrane domains instead of **TMHMM**, so to save time we will import already [precomputed results](link here!) and integrate them in the pipeline. To retrieve proteome, we will use the most recent version of [**biomartr**]((https://cran.r-project.org/web/packages/biomartr/index.html) package.
...
# Predict signal peptides, check additional targeting signals and ER-motifs, integrate TOPCONS predictions
pipeline_1 <- signalp(pinf,
                      version = 4,
                      organism = 'euk',
                      sensitive = TRUE,
                      run_mode = 'starter',
...
# use the resulting file to predict TM doamins with TOPCONS, either stand-alone version, WSDL-API or WEB-server
writeXStringSet(pipeline_1, "pinf_secretome_pt1.fasta")

# pre-computed results could be found here:
...
Now we will download all available fungal proteomes using [``biomartr``](https://cran.r-project.org/web/packages/biomartr/index.html) package and apply the same standard secretome prediction pipeline. To speed-up the process, we will use 20 cores available on our local HPC. Please note, retrieval of proteomes might take a while.
