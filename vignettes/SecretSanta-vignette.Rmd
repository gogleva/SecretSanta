...
The **SecretSanta** package attempts to bridge this gap. It provides wrapper
...
the initial submitted sequences and an ``out_fasta`` slot with positive candidates after the method application. Each particular method then complements this simple structure with relevant slots. For example, outputs of the ``signalp`` function are organised in ``SignalpResult`` objects. Apart from ``in_fasta`` and ``out_fasta`` slots inherited from ``CBSResult`` class, ``SignalpResult`` objects contain three additional slots, relevant for the **signalp** method:
...
facilitate comparisons of secretomes across multiple species or under various conditions. For example, an earlier draft of this package was successfully used to compare the secreted protein complement of a plant pathogen throughout an infection time-course in plant roots (Evangelisti et al. 2017)
...
**SecretSanta** relies on a set of existing command line tools to predict secretd proteins. Please install them and configure according to the listed instructions. Due to limitations imposed by the external dependencies, some of **SecretSanta** wrapper functions will nott work on Windows, however are fully fuctional on Linux and Mac.
...
```
Please note that ``signlap`` truncates all the input sequences
...
mitochondria and is indeed extracellular. The ``targetp`` function accepts input
...
The ``wolfpsort`` function accepts objects of ``CBSResult`` class with non-empty
...
run these functions side by side tp compare the obtained results
...
Say, we would like to have a relatively short pipeline (**Figure 2**) with the
...
c4 <- "\\label{fig:pipe1} **Figure 4.** Example of a stringent pipeline."
