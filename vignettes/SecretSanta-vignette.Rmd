...
slots could be extracted at once with `getFastas` method, organised in `AAStringSetList` object. This makes it possible to apply all the defined `AAStringSetLisst` methods to the extracted object.
...
Each particular method then complements this simple structure with relevant slots. For example, outputs of the ``signalp`` function are organised in ``SignalpResult`` objects. Apart from ``in_fasta`` and ``out_fasta`` slots inherited from ``CBSResult`` class, ``SignalpResult`` objects contain three additional slots, relevant for the **signalp** method:
...
input sequences exceeds a certain limit. The package also contains a convinience `ask_uniprot` function - to export known information about subcellular localisation. This can allow the user to compare/verify predictions and is mostly applicable for well-annotated and curated genomes/species.
