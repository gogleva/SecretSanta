---
title: 'SecretSanta: flexible pipelines for automated secretome prediction'
author: "Anna Gogleva"
date: '`r Sys.Date()`'
...
i.e. utilizing dedicated signal peptides.
...
identify secreted proteins as shown in **Figure 1 a** and filer out
those shown in **Figure 1 b-d**.

\newline

```{r motifs, out.width="400px", fig.align="center", echo=FALSE, fig.cap=c1}

c1 <- "\\label{fig:motifs} **Figure 1.** Characteristic motifs, domains and 
their arrangements, helping to distinguish extracellular proteins from 
proteins retained inside the cell."
...
A number of excellent command line tools and web-interfaces are available to perform predictions of individual motifs and domains
...
```{r objects, out.width="600px", fig.align="center", echo=FALSE, fig.cap= c2}
c2 <- "\\label{fig:motifs} **Figure 2.** SecretSanta class structure."
...
check1 <- manage_paths(in_path = TRUE, test_mode = 'signalp2')
```
...
the number if CPUs available and the input file size. With 48 CPUs it takes ~2
...
wlf <- wolfpsort(step1_sp2, organism = 'fungi')
```
...
run these functions side by side and compare the obtained results
...
er_result <- check_khdel(step1_sp2, run_mode = 'piper')
...
would like to have a relatively short pipeline (**Figure 2**) with the
...
```{r pipe1, out.width = "200px", fig.align="center", echo = FALSE, fig.cap=c3}
c3 <- "\\label{fig:pipe1} **Figure 3.** Minimal pipeline for secretome 
prediction"
...
```{r pipe2, out.width = "300px", fig.align="center", echo = FALSE, fig.cap=c4}
c4 <- "\\label{fig:pipe1} **Figure 3.** Example of a stringent pipeline."
